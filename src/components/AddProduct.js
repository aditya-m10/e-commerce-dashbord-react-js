...
const Addproduct=()=>{
    ...
    return (
        <div className="box">
            ...
            {error && !name && <small className="error-input">Enter valid name</small>}
            <input value={price} onChange={(e)=>setPrice(e.target.value)} className="inputBox" type="number" placeholder="Price" />
            {error && !price && <small className="error-input">Enter valid price</small>}
            <input value={category} onChange={(e)=>setCategory(e.target.value)} className="inputBox" type="text" placeholder="Category" />
            {error && !category && <small className="error-input">Enter valid category</small>}
            <input value={company} onChange={(e)=>setCompany(e.target.value)} className="inputBox" type="text" placeholder="Company name" />
            {error && !company && <small className="error-input">Enter valid company</small>}
            <button  onClick={addProduct} className="button-product inputBox" ><a >Add Product</a></button>
